...
class ReportsController extends ControllerCore
{
		...
		if($table == 'txn_stock_transfer_in_header' && $stockTransNum && $column == 'stock_transfer_number')
		{
			...
			if($transfer)
			{
				...
				\DB::table('table_logs')->insertGetId([
								...
								'comment' => $comment,
								'report_type' => $report_type,
								'salesman_code' => $this->getSalesmanCode('txn_stock_transfer_in_detail',$transfer->stock_transfer_in_detail_id)
						]);
			}
	...
	/**
	 * Get Salesman Code of a table
	 * @param  String $table [table to reference]
	 * @param  String $pk_id [primary key of the table]
	 * @return String [returns the salesman_code]
	 */
