...
class ReportsPresenter extends PresenterCore
{
    ...
    public function getPreparedSalesCollection($summary = false, $noInvoice=false)
    {
    	$query = ' SELECT
				   ...
				   coalesce((sotbl.so_total_served - sotbl.so_total_collective_discount),0.00) total_invoice_amount,
				   ...
				   (coalesce((sotbl.so_total_served - sotbl.so_total_collective_discount),0.00) - coalesce(sotbl.so_total_ewt_deduction, 0.00) - coalesce((rtntbl.RTN_total_gross - rtntbl.RTN_total_collective_discount),0.00)) total_invoice_net_amount,
    ...
    public function getPreparedSalesReportMaterial($summary=false)
    {
    	...
    	$select = '
				...
				sales.vat_amount,
				CONCAT(coalesce(sales.discount_rate,0.00),\'%\') discount_rate,
				sales.discount_amount,
    			CONCAT(coalesce(sales.collective_discount_rate,0.00),\'%\') collective_discount_rate,
    ...
    public function getPreparedSalesReportPeso($summary=false)
    {
    	$querySales = '
				...
				all_so.gross_served_amount,
				all_so.vat_amount,
				CONCAT(coalesce(all_so.discount_rate,0.00),\'%\') discount_rate,
				all_so.discount_amount,
			    CONCAT(coalesce(all_so.collective_discount_rate,0.00),\'%\') collective_discount_rate,	
			    all_so.collective_discount_amount,
			    all_so.discount_reference_num,
			    all_so.discount_remarks,			    	
			    all_so.total_sales total_invoice,
